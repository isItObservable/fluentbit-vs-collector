# The Dynatrace dashboard — state and why

**Current version: 7** (2026-07-23). Source of truth:
`resource-comparison.dashboard.json` in this directory — import it into your own
Dynatrace environment and substitute `${CLUSTER_NAME}` in the tile queries.

The dashboard is the surface a run gets read from, so a wrong tile here becomes
a wrong published number. This file records what was changed and why, because
the *why* is not obvious from the DQL.
...
than `the benchmark cluster` stamps it on logs — so it discriminates safely
