# ISI-1817 — df_engine 0.50.0 attribute-landing probe

**Date:** 2026-07-23, 10:09–10:12 UTC
**Cluster:** `observable-agentsandbox` — **not** the measured cluster.
**Image:** `ghcr.io/isitobservable/df_engine:0.50.0` (the R1P3 image, unchanged)
**Manifest:** `probe.yaml` · **Raw evidence:** `sink.log`

## Why this ran

`c4fe469` rewrote CHECK 5b's arrow branch to assert span and log liveness on
...
**Flagged for @BigBoss's consolidation, not fixed:** the config is frozen, and
...
cluster `observable-otelarrow` was never contacted during this work.
