...
| `fluentbit-v5` | `spans=SAFE logs=UNSAFE metrics=SAFE` | spans/logs measured over the banked R1P2 window: spans 17,858,597/17,858,597 · logs **0**/10,263,977. **metrics: NO-DATA → SAFE from Round 2 (board decision D15, 2026-07-26).** R1P2 shipped 0 metric series — a config defect (metrics content_modifier missing `context:`), fixed in `d655566` effective R2 onward. R1P2's register row keeps `NO-DATA` (historical, not back-dated); the R1↔R2 metrics-path non-comparability is a RESULTS finding |
| `otel-collector` | `spans=SAFE logs=SAFE metrics=SAFE` | **Measured at R2P1** (ISI-1818, 2026-07-23): spans 481,408/481,408, logs 262,136/262,136, metrics 1/1 series, all 100.00%. Both `UNKNOWN`s are now real readings and were written back into `expected_for()` as the runbook required. Read it against the fluentbit row: same attribute, same cluster, **0%** on logs there — cluster tagging on logs is an engine property, not an environment property |
...
off-cluster with the pipeline config on 2026-07-23 and pushed 50 records per
...
`spans=SAFE logs=SAFE metrics=NO-DATA` for the arrow arm — unlike fluentbit, its log
tiles need no cluster-filter correction.

⚠️ That probe measured the **old single-chain** config, where metrics still reached the
exporter; this paragraph said `metrics=SAFE` until 2026-07-23. The corrected arm routes
metrics to `exporter:noop` (board Q2, ISI-1841), so `NO-DATA` is the correct expectation
and `SAFE` would false-FAIL a healthy engine. The table above was fixed in `6be680b` and
**this copy was missed** — the third time in this campaign a correction landed in one
place and not in its duplicate. The single source of truth is `expected_for()` in
`results/attr-landing.sh`; both this paragraph and CHECK 5b now defer to it.
