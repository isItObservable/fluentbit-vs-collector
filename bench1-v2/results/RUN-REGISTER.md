...
| `R1-P3-arrow` | `ghcr.io/isitobservable/df_engine:0.50.0` | 1 | `observable-otelarrow` | 1 | `DNF` | `DNF` | `DNF — never reached` | `DNF` | `DNF` | `rampup2h` 50→100→150→200 VU, both apps — **never applied** | **🛑 DNF — declared 2026-07-23 under ISI-1849, on the board's pre-authorisation of 2026-07-23 13:45Z. This row will never be filled.** The arm aborted at the validation gate twice, before any timed load, on smoke-level traffic: attempt 1 (`results/r1p3-abort/`) lost all 4 pipeline cores by T+30.3s; attempt 2 (`results/r1p3-retry/`, running the ISI-1843-corrected config with provenance verified) by T+33m34.2s — 68x longer, same ending. Cause: `DictionaryKeyOverflowError` at two independent concatenation sites, `crates/pdata/src/otap/transform/concatenate.rs:150` (fires first) and `arrow-data/src/transform/mod.rs:680`. Accumulation-driven, so a 120-min ramp to 200 VU is further out of reach than the smoke that killed it, not closer. Rebuilding from newer upstream was checked and rejected on evidence, not spent: all 13 commits in `7502e7d..main` (to `51b864b9`, 2026-07-23) leave those paths untouched — see `results/r1p3-dnf/STEP0-UPSTREAM-DELTA.md`. Upstream report open-telemetry/otel-arrow#3561. **Full declaration and scope: `results/r1p3-dnf/DNF.md`.** R1P1 and R1P2 are unaffected and remain comparable to each other; the campaign is two engines, not three. |
| `R2-P1-collector` | otel-collector contrib `0.154.0` | 2 | `observable-otelarrow` | 1 | `2026-07-23T17:01:13Z` | ⟨UNSET — RUN IN FLIGHT, ends ~19:01Z⟩ | `PASS 8/8 @ 2026-07-23T17:00:54Z` | ⟨UNSET — Start census recorded below⟩ | `spans=SAFE logs=SAFE metrics=SAFE` — **first measurement of logs and metrics on the collector arm** (both were `UNKNOWN` in `expected_for()`); spans 481,408/481,408, logs 262,136/262,136, metrics 1/1 series, all 100.00%. Contrast R1P2 fluentbit `logs=UNSAFE` (0/10,263,977) — the collector's log tiles need no read-time cluster-filter correction. | `rampup2h` 50→100→150→200 VU, both apps | **Round 2 replication of `R1-P1-collector`. Fresh deploy of everything** — engine, both apps, Istio reconfigured and istiod rolled, all namespaces restarted. Start stamped **after** the ramp apply returned success (all 8 Jobs created), not before it. Gate green on the first attempt, 8/8, with no hand-patching: `apps/appprotocol.sh` patched=35 already-correct=0 failed=0 absent=0 (helm recreates the Services every phase). CHECK 5c forward-window liveness: 90,751 records delivered inside the 120 s sample. CAAPH reconciliation for istiod remains PAUSED for the campaign (ISI-1826); `hipster-shop/loadgenerator`, the pre-campaign constant, was left running and untouched. |
...
| `R2-P3-arrow` | `ghcr.io/isitobservable/df_engine:0.50.0` | 2 | `observable-otelarrow` | 1 | `CANCELLED` | `CANCELLED` | `CANCELLED` | `CANCELLED` | `CANCELLED` | — | **Cancelled with the arm (ISI-1820).** Round 2 replicates Round 1; there is no R1P3 to replicate. Soak S3 (ISI-1824) cancelled for the same reason — the engine dies in ~26 min under this workload, so a 24h soak has nothing to measure. See `results/r1p3-dnf/DNF.md`. |
...
| `R2-P3-arrow` | — | `CANCELLED` | `CANCELLED` | `CANCELLED` | `CANCELLED` | **`CANCELLED`** with the arm (ISI-1820). |
