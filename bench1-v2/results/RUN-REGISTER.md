...
| `S3-otel-arrow` (attempt 3, ISI-3301) | `ghcr.io/isitobservable/df_engine:0.51.0` @ `sha256:0f85174d…` | `observable-otelarrow` | 1 | `2026-08-26T16:42:37Z` | _pending — driver ends ~2026-08-27T16:42:37Z_ | `PASS 8/8 @ 2026-08-26T16:42:37Z` (attempt-3 driver; first gate run false-RED on DT ingestion lag — see Notes) | `Start: bench-otel-arrow-native-6779dc9794-psb7t @2026-08-26T16:27:47Z, worker lz9mb, 0 restarts, pods=1 — End pending` | SAFE (spans/logs; metrics NO-DATA by design Q2) | `leak24h` 50 VU per app, both apps | _pending_ | **Attempt 3 OPEN.** Full §1 cycle re-staged 2026-08-26 (fresh engine deploy, otel-demo + hipster-shop arrow variants, appprotocol, ns labels, Istio telemetry + istiod restart). All 3 workers Ready (9r662 recovered); SPOF mitigation via scheduler spread — engine @lz9mb, soakjob-hipster @9r662, soakjob-oteldemo @j82ph. ⭐ First gate run (16:21:53Z) went 4/8 RED with zero spans/logs in-window; post-hoc re-query of the SAME window showed 24,505 app spans / 13,773 logs / 8,445 istio-mesh spans — **DT ingestion lag (~2–3 min) raced the fresh engine pod**. Driver patched to retry the gate (≤3 attempts, 300s settle); re-run gate went 8/8 GREEN attempt 1. Driver detached under `/tmp/s3-arrow-driver.lock` (flock); artifacts → `_artifacts/isi1779/soak/S3-arrow`. |
