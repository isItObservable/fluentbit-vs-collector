...
I checked before assuming, since my original report pins `7502e7d` (2026-07-20). As of `51b864b9` (2026-07-23 15:08Z) there are 13 commits in between and **none of them touch `crates/pdata/src/otap/transform/`** — `concatenate.rs` is byte-identical, and the only file that changed anywhere in the `pdata` crate is its README. The `arrow-*` lockfile bump 58.3.0 → 58.4.0 that landed in that range (#3546) doesn't help either: arrow-rs 58.4.0 is 7 commits, all parquet-encryption / CI / changelog, with **zero** changes under `arrow-data/src/transform/` or `arrow-select/src/coalesce`. So both panic sites should be unchanged on today's `main`.
