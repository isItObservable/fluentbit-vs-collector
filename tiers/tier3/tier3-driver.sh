#!/usr/bin/env bash
# ISI-3577 E3 — Tier 3 (logs + metrics + TRACES, NO tail sampling) — SERIAL SINGLE-ENGINE
...
#   T+26h -> 24h soak readout: same set -> POST, then teardown bench-load, flag ARM 2 (FB).
# Tier delta vs Tier-2 = a TRACES pipeline; telemetrygen @200 span/s drives the engine OTLP receiver.
set -uo pipefail
...
ISSUE="658650ae-4891-4893-9b69-c607bd594802"   # ISI-3577
...
readout() { # $1 phase ; $2 outfile
  { echo "# Tier 3 (logs+metrics+traces, NO tail sampling) — ARM 1 collector v0.159.0 — $1 readout"
...
T0=$(date -u +%FT%TZ); echo "[driver] Tier3 ARM1(collector)+LOAD START T0=$T0 pid=$$"
echo "[driver] applying load (locust+k6+telemetrygen traces/metrics) at T0"; apply_load

sleep 7200
GATE="$OUT/tier3-collector-2h-gate.md"; readout "2h validity gate" "$GATE"
if green "$GATE"; then GV="🟢 GREEN — census PASS + logs/metrics/traces in Dynatrace + load reaching app"; else GV="🔴 RED — a gate failed (dataset INVALID)"; fi
post "$(printf '## Tier 3 · ARM 1 (collector) — 2h validity gate: %s\n\nT0=%s · gate at %s · SINGLE-ENGINE (pt9), FB offline · signals=logs+metrics+traces, NO tail sampling · load=locust+k6+telemetrygen(traces@200)\n\n```\n%s\n```\n\nArtifact: `tiers/tier3/tier3-collector-2h-gate.md`. %s' \
  "$GV" "$T0" "$(date -u +%FT%TZ)" "$(sed -n '1,64p' "$GATE")" \
...
sleep 86400
FINAL="$OUT/tier3-collector-results.md"; readout "24h soak" "$FINAL"
...
echo "[driver] Tier3 ARM1 DONE — FB arm is the next heartbeat's manual step"
