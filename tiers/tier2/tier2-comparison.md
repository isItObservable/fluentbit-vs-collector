# Tier 2 (logs + metrics) — E2 per-engine comparison → feeds E5 (ISI-3579)

**Signal set:** LOGS + METRICS. **Topology:** DaemonSet (filelog logs) + StatefulSet (metrics: prometheus scrape of istiod CP + Kepler, cumulative→delta, drop-summary). Fluent Bit arm uses FB's native metric-conversion path for the metrics pipeline. **Methodology:** pt9 serial single-engine — one engine live at a time, exporting real logs+metrics to Dynatrace; validation gate = signal RECEIVED IN DYNATRACE + census PASS + load reaching app. **Cluster:** observable-otelarrow. **Load:** locust(otel-demo) + k6(hipster-shop).
...
| Loss (accepted vs sent) | **NO LOSS** — logs refused=0/send_failed=0; metrics refused=0/send_failed=0 (metrics sent<accepted EXPECTED: cumulative→delta drops 1st sample/series + drop-summary, ISI-1843) | **NO LOSS** — in 43,440,978 / out 43,440,657 / dropped=0 / loss%=0.0000 | tie |
...
## Kepler high-cardinality cost delta — the E2-specific watch item
...
- **Fluent Bit v5 is the lighter logs+metrics engine at this scale**, and the gap is widest on the **metrics arm** (~11× lower working set on the Kepler/istiod scrape pipeline). Directionally consistent with Tier 1 (E1: FB ~5× lighter memory, ~30% lower CPU/cost on logs-only) and the ISI-1779 clean benchmark.
...
- ARM 1 collector 24h COMPLETE VALID: issue comment 2026-09-09T08:51Z · raw readout in this comment thread.
- ARM 2 fluent-bit 24h COMPLETE VALID: issue comment 2026-09-10T~11:09Z · raw readout `tier2-fluentbit-results.md` (this dir); driver logged HTTP 201 post + bench-load teardown.
- Executed under ISI-3576 (E2), extended-benchmark epic ISI-3572. Feeds E5 / ISI-3579.
