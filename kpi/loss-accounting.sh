#!/usr/bin/env bash
# loss accounting (accepted == sent).
# Note: received != exported is COALESCING, not loss — but for a raw
...
case "$ARM" in
  collector)
    ...
    # Note: sent can EXCEED accepted (fan-out to N exporters/pipelines) — a raw
