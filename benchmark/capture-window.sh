#!/usr/bin/env bash
# B1-v2 — recover a run's measurement window from Kubernetes itself.
#
...
# teardown". The timed run is 120 minutes; an agent heartbeat is 30.
...
#
# It does NOT need a capture loop (methodology rule D8 forbids one, and a loop
# perturbs what it measures). Kubernetes already records the instant every ramp
# pod stopped, to the second, for free.
#
# WHICH FIELD -- measured live on the benchmark cluster 2026-07-22T14:40Z:
#
...
#      run on a stray 500 (13 x 500 out of 78,258 requests marked a
...
export KUBECONFIG="${2:-${KUBECONFIG:?set KUBECONFIG to the benchmark cluster}}"
SEL='ramp=logship'
