...
#
# Requires: kubectl (KUBECONFIG -> the benchmark cluster), dtctl (context with
# Grail read on the the Dynatrace tenant tenant).
# ============================================================================
set -uo pipefail
...
# Every DQL query in this gate is scoped by cluster (methodology rule D12).
# k8s.workload.name and the k8s.* metric keys are NOT unique across the clusters
# reporting into this tenant — another cluster and a third cluster emit
...
# Sidecar exclusions (the methodology check 3). otel-demo's load-generator and
# frontend-proxy are excluded by the Helm values; the ramp-job load drivers
# exclude themselves and are matched by their `ramp: logship` label.
...
# but was a string" (validated against the Dynatrace tenant). Do not copy the
...
say "B1-v2 phase validation gate"
...
for p in d.get("items",[]):
    ...
    if rx.match(name) or m.get("labels",{}).get("ramp")=="logship": continue
    expect+=1
    ...
    # spec.containers. Verified live on the benchmark cluster (k8s v1.35.3):
...
# ---------------------------------------------------------------------------
# 4a/4b/4c — CONFIG PREFLIGHT, added after R1P1  burned two runs on
...
print(sum(v.get("proc_records", 0) for v in d.get("output", {}).values()))' 2>/dev/null || echo 0)
  ...
  # -------------------------------------------------------------------------
  # 5b PER-SIGNAL health — added 2026-07-23 after R1P2 
  # -------------------------------------------------------------------------
  ...
  #
  # Same shape as CHECK 2's per-namespace rule : an aggregate assertion
...
' 2>/dev/null) || sigres="FAIL|per-signal scrape failed"
  ...
  # -------------------------------------------------------------------------
  # 5c IS IT STILL ALIVE? — added 2026-07-23 after R1P3 
  # -------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# CHECK 6 — pod census baseline (methodology rule D12)
# ---------------------------------------------------------------------------
...
# Re-proven live on the benchmark cluster over 2026-07-20T11:00Z -> 2026-07-22T11:00Z:
